...
# Consulta ao APASS DR9 (banda B)
centro_campo = SkyCoord(ra=df['RA_deg'].mean() * u.deg, dec=df['Dec_deg'].mean() * u.deg)
viz = Vizier(columns=['RAJ2000', 'DEJ2000', 'Bmag', 'e_Bmag'], row_limit=-1)
consulta = viz.query_region(centro_campo, radius=15.0 * u.arcmin, catalog='II/336/apass9')[0]

cat_apass = consulta[~np.isnan(consulta['Bmag'])]
sel_mag = (cat_apass['Bmag'] > mag_lim_apass[0]) & (cat_apass['Bmag'] < mag_lim_apass[1])
...
m_cat_vals = cat_apass['Bmag'][idx_cat[m_validos]]
...
df['Mag_B_Calibrada'] = reta(df['Mag_Inst'], alfa, c)
...
print(f"Ajuste: B = ({alfa:.4f} +/- {e_alfa:.4f}) * m_inst + ({c:.4f} +/- {e_c:.4f})")
...
plt.plot(
    ...
    label=rf'$B = ({alfa:.3f} \pm {e_alfa:.3f}) \cdot m_{{inst}} + ({c:.2f} \pm {e_c:.2f})$'
)
...
plt.ylabel('Magnitude Aparente APASS DR9 ($B$)', fontsize=11)
plt.title('Calibração Fotométrica - Banda B (APASS DR9)', fontsize=12, pad=10)
...
plt.xlabel("Magnitude Aparente Catálogo ($B$)")
...
plt.xlabel('Magnitude Calibrada (B)', fontsize=12)
plt.ylabel('Número de Estrelas (N)', fontsize=12)
plt.title('Distribuição de Magnitudes e Profundidade (Banda B)', fontsize=13, pad=12)
