...
# ====================================================================
# 1. Parâmetros e Arquivos (Filtro G)
# ====================================================================
INPUT_CSV_TOTAL = 'fotometria_final_G.csv'          
INPUT_CSV_FILTRADO = 'fotometria_limpa_G.csv'  
OUTPUT_CSV = 'fotometria_G_calibrada.csv'
...
gaia = vizier.query_region(centro, radius=15.0 * u.arcmin, catalog='I/355/gaiadr3')[0]

# Limite do Gaia (o catálogo alcança estrelas bem mais fracas)
gaia = gaia[(~np.isnan(gaia['Gmag'])) & (gaia['Gmag'] > 8.0) & (gaia['Gmag'] < 20.0)]
...
y_fit = gaia['Gmag'][idx_filt[mask_filt]].value
...
y_tot = gaia['Gmag'][idx_tot[mask_tot]].value
...
df_total['Mag_G_Calibrada'] = ajuste_linear(df_total['Mag_Inst'], alfa, c)
...
print("\n--- Resultados do Ajuste Linear (Gaia G) ---")
...
print(f"Equação: G_Gaia = ({alfa:.4f}) * Mag_Inst + ({c:.4f})")
...
plt.title('Calibração Fotométrica G: Instrumental vs Gaia DR3', fontsize=13)
plt.xlabel('Magnitude Instrumental (Mag_Inst + 25)', fontsize=11)
plt.ylabel('Magnitude Aparente (Gaia G)', fontsize=11)
plt.legend(loc='best')
plt.grid(True, linestyle=':', alpha=0.6)
plt.tight_layout()

plt.savefig('grafico_calibracao_G_Gaia_DS9_limite.png', dpi=300)
plt.show()