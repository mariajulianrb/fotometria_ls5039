...
# Carrega os dados e aplica ZP inicial
...
# Consulta APASS DR9 (banda B) no VizieR
...
# Filtra nans e intervalo de magnitude
...
# Aplica calibracao na tabela inteira
...
plt.errorbar(
    ...
    fmt='.', color='gray', alpha=0.35, label='Fontes pareadas'
)
...
plt.errorbar(
    ...
    capsize=2, alpha=0.8, markeredgecolor='k', label=f'Inliers (Top {n_top})'
)
...
plt.axvline(x=m_inst_5sig, color='purple', linestyle=':', alpha=0.7)
plt.axhline(
    y=mag_lim_5sig, color='purple', linestyle=':', alpha=0.7,
    label=f'Limite 5$\sigma$ ({mag_lim_5sig:.2f} mag)'
)

plt.xlabel('Magnitude Instrumental ($ZP = 25$)')
...
# 3. Histograma das magnitudes calibradas
