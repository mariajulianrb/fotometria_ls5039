...
print(f"Limite 5-sigma: {mag_limite_5sig:.2f} +/- {err_mag_lim_5sig:.2f} mag")
...
plt.xlabel('Magnitude Instrumental', fontsize=11)
plt.ylabel('Magnitude Aparente APASS DR9 ($R$)', fontsize=11)
...
plt.scatter(
    y_fit[inliers], 
    residuos_fit[inliers], 
    color='purple', alpha=0.7, edgecolor='k', linewidth=0.8, zorder=3, label=f'Inliers'
)

plt.axhline(0, color='black', linestyle='--', linewidth=1.5, zorder=1)

plt.xlabel("Magnitude Aparente ($R$)")
plt.ylabel('Resíduo')
plt.title('Resíduos do Ajuste Linear — Banda R', fontsize=12, pad=10)
...
plt.axvline(
    ...
    linestyle='--', 
    linewidth=2.5, 
    label=rf'Limite 5-$\sigma$ ({mag_limite_5sig:.2f} $\pm$ {err_mag_lim_5sig:.2f} mag)'
)

plt.xlabel('Magnitude Calibrada (R)', fontsize=12)
plt.ylabel('Número de Estrelas (N)', fontsize=12)
plt.title('Distribuição de magnitudes e limite instrumental', fontsize=13, pad=12)
